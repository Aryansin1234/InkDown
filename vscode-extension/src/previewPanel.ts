...
export class PreviewPanel {
  ...
  private lastEditorScrollLine = -1;
  ...
  private constructor(
    ...
    this.disposables.push(
      this.panel.webview.onDidReceiveMessage((msg) => {
        ...
        if (msg.type === 'scrollSync' && typeof msg.line === 'number') {
          const editor = vscode.window.visibleTextEditors.find(
            (e) => e.document.languageId === 'markdown' && e.viewColumn !== this.panel.viewColumn
          );
          if (editor) {
      ...
      vscode.window.onDidChangeTextEditorVisibleRanges((e) => {
        if (this.ignoreEditorScroll) { return; }
        if (
          ...
          if (topLine === this.lastEditorScrollLine) { return; }
