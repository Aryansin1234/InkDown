...
function resolveElements() {
  el = {
    ...
    cssThemeInput:    document.getElementById('cssThemeInput'),
    cssThemeName:     document.getElementById('cssThemeName'),
...
function initOptions() {
  ...
  if (el.formatToggle) {
    el.formatToggle.addEventListener('click', (e) => {
      ...
      const labelMap = { pdf: 'Generate PDF', docx: 'Generate DOCX', html: 'Generate HTML' };
      const infoMap  = { pdf: 'PDF renders in ~5 seconds', docx: 'DOCX renders in ~2 seconds', html: 'HTML renders in ~3 seconds' };
      if (el.convertBtnLabel) el.convertBtnLabel.textContent = labelMap[fmt] || 'Generate';
      if (el.convertInfoText) el.convertInfoText.textContent = infoMap[fmt] || 'Rendering…';
    });
  }

  // Advanced options
  if (el.authorInput)    el.authorInput.addEventListener('input',    () => { state.options.author    = el.authorInput.value.trim(); });
  if (el.pageSizeSelect) el.pageSizeSelect.addEventListener('change', () => { state.options.pageSize  = el.pageSizeSelect.value; });
  if (el.watermarkInput) el.watermarkInput.addEventListener('input',  () => { state.options.watermark = el.watermarkInput.value.trim(); });
  if (el.cssThemeInput) el.cssThemeInput.addEventListener('change', () => {
    const file = el.cssThemeInput.files[0];
    state.options.cssThemeFile = file || null;
    if (el.cssThemeName) el.cssThemeName.textContent = file ? file.name : 'Upload .css…';
    const label = document.getElementById('cssThemeLabel');
...
async function handleConvert() {
  ...
  if (state.options.author)    fd.append('author',    state.options.author);
  if (state.options.pageSize && state.options.pageSize !== 'A4') fd.append('pageSize', state.options.pageSize);
  if (state.options.watermark) fd.append('watermark', state.options.watermark);
  if (state.options.cssThemeFile) fd.append('cssTheme', state.options.cssThemeFile);
  ...
  try {
    ...
    if (state.mode !== 'upload' && !state.options.cssThemeFile) {
    ...
    const extMap = { pdf: 'pdf', docx: 'docx', html: 'html' };
    const ext = extMap[state.format] || 'pdf';
    ...
    const mimeType = mimeMap[state.format] || 'application/octet-stream';
...
function setLoading(isLoading) {
  ...
  if (isLoading) {
    ...
    const msgs = state.format === 'pdf'
      ...
      : state.format === 'html'
      ? ['Parsing Markdown…', 'Rendering HTML…', 'Inlining assets…', 'Finalising…']
      : ['Parsing Markdown…', 'Finalising…'];
