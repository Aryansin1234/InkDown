...
function registerCommands(context, serverManager) {
    ...
    // ── Batch export (E2) ──────────────────────────────────
    vscode.commands.registerCommand('inkdown.batchExportFolder', async (uri) => {
        ...
        const format = config.get('defaultFormat', 'pdf');
        const outDir = config.get('outputDirectory', '') || folderUri.fsPath;
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `InkDown: Exporting ${mdFiles.length} files…`, cancellable: false }, async (progress) => {
            const cli = new inkdownClient_1.InkDownClient(serverManager);
            let done = 0;
            let errors = 0;
            ...
            vscode.window.showInformationMessage(`InkDown: Batch export complete — ${done} succeeded${errors > 0 ? `, ${errors} failed` : ''}.`);
        });
    }), 
    // ── Toggle auto-export (E1) ────────────────────────────
    vscode.commands.registerCommand('inkdown.toggleAutoExport', () => {
        const config = vscode.workspace.getConfiguration('inkdown');
        const current = config.get('autoExport', false);
        config.update('autoExport', !current, vscode.ConfigurationTarget.Global);
        vscode.window.showInformationMessage(`InkDown: Auto-export on save is now ${!current ? 'enabled' : 'disabled'}.`);
    }));
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (doc) => {
        ...
        const format = config.get('autoExportFormat', 'pdf');
        const outDir = config.get('outputDirectory', '') || path.dirname(doc.uri.fsPath);
        ...
        try {
            ...
            vscode.window.showErrorMessage(`InkDown: Auto-export failed — ${e instanceof Error ? e.message : String(e)}`);
        }
    }));
}
...
async function runConvert(uri, format, client, promptOptions) {
    ...
    let options = {
        format: (format ?? config.get('defaultFormat', 'pdf') ?? workspaceConfig.format ?? 'pdf'),
        title: baseName,
        toc: workspaceConfig.toc ?? config.get('defaultToc', false),
        autoBreak: workspaceConfig.autoBreak ?? config.get('defaultAutoBreak', false),
    ...
    const outDir = config.get('outputDirectory', '') || workspaceConfig.outputDirectory || path.dirname(sourcePath);
 ...
 */
function upsertFrontmatter(filePath, updates) {
...
async function pickTheme(client, current, title) {
    const themesDir = await client.getThemesDir();
    const items = BUNDLED_THEMES.map((t) => {
...
async function gatherOptions(defaults, sourcePath, client) {
    // ── Step 1: Format ──────────────────────────────────────────
    const formatItems = [
    ...
    if (extras.some((e) => e.id === 'writeFm')) {
        const fmUpdates = {};
        if (title.trim()) {
            fmUpdates.title = title.trim();
        }
        if (author.trim()) {
            fmUpdates.author = author.trim();
        }
        if (date.trim()) {
            fmUpdates.date = date.trim();
        }
        if (watermark.trim()) {
            fmUpdates.watermark = watermark.trim();
        }
