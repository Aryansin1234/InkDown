...
function activate(context) {
    ...
    serverManager.detectInkDownPath().then((found) => {
        if (!found) {
            vscode.window
                .showWarningMessage('InkDown: Could not find an InkDown installation in this workspace. Configure `inkdown.inkdownPath` in settings.', 'Open Settings')
                .then((action) => {
                if (action === 'Open Settings') {
                    vscode.commands.executeCommand('workbench.action.openSettings', 'inkdown.inkdownPath');
                }
            });
        }
    });
